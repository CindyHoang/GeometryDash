...
//    }
    ...
    obstacles.push_back(new Enemy(1.7, -0.5));
...
//    obstacles.push_back(new Triangle(1.6, -0.5));
	...
	for (int i = 0; i < 3; i++) {
		obstacles.push_back(new Obstacle(4.2 + (0.1*i), -0.3 + (-0.1*i)));
	}
    for (int i = 0; i < 3; i++) {
    ...
    for (int i = 0; i < 5; i++) {
        obstacles.push_back(new Obstacle(6.0+(0.1*i), -0.2));
    }
    for (int i = 0; i < 2; i++) {
...
//    }
    powerups.push_back(new Powerup(0.8, -0.3, 1));
    powerups.push_back(new Powerup(1.5, -0.2, 0 ));
//    for (int i = 0; i < 5; i++) {
...
void App::draw() {
    ...
    if (welcome != 0) {
        if (gameover) {
			...
			drawString(GLUT_BITMAP_HELVETICA_18, "Score: ", -0.175, -0.2, 0);
			drawString(GLUT_BITMAP_HELVETICA_18, pts, 0.08, -0.2, 0);
...
//        delay += 0.1f;
		...
		//teal powerup
		if (p->getR() == 0.0f && p->getG() == 1.0f && p->getB() == 1.0f) {
			jumpHeight = 0.45f;
			delay += 0.1f;
...
//        }
        ...
        for (int i = 0; i < obstacles.size(); i++) {
            ...
            obstacles[i]->setX(x - 0.0125f * speed);
            score += 0.01;
			...
			if (obstacles.begin() == obstacles.end()) {
				welcome = 1;
				missileX = 0.74;
				missileY = -0.55;
				p->resetColor();
        ...
        for (int i = 0; i < powerups.size(); i++) {
            ...
            powerups[i]->setX(x - 0.007f*speed);
			...
			// purple powerup
			else if (p->getR() == 1.0f && p->getG() == 0.0f && p->getB() == 1.0f) {
				speed = .75f;
			}
